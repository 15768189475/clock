...
void TSC_ConfigIO(TSC_ConfigIO_T* tscConfigIO)
{
    ...
    TSC->IOHC = (~(tscConfigIO->IOChannel | tscConfigIO->IOShield | tscConfigIO->IOSampling));

    /** Set channel and shield IOs */
    TSC->IOCC = (tscConfigIO->IOChannel & (~tscConfigIO->IOShield));

    /** Set sampling IOs */
    TSC->IOSC = tscConfigIO->IOSampling;
...
uint16_t TSC_ReadGroupValue(TSC_GROUP_T group)
{
    ...
    {
        case 1:
            value = TSC->IOG1C_B.CNTVAL;
            break;

        case 2:
            value = TSC->IOG2C_B.CNTVAL;
            break;

        case 3:
            value = TSC->IOG3C_B.CNTVAL;
            break;

        case 4:
            value = TSC->IOG4C_B.CNTVAL;
            break;

        case 5:
            value = TSC->IOG5C_B.CNTVAL;
            break;

        case 6:
            value = TSC->IOG6C_B.CNTVAL;
            break;
    }
